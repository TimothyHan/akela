...
const CONFIG_KEYS = new Set(['domain', 'knowledge', 'learnings', 'runs', 'compiler', 'activities', 'profile', 'fingerprints', 'statuses', 'scratchpad', 'idTag']);
const COMPILER_KEYS = new Set(['scope', 'scoring', 'budget_lines', 'scoringOverride', 'retrievers']);
const PACK_KEYS = new Set(['name', 'activities', 'profile', 'fingerprints', 'statuses', 'scratchpad', 'knowledge', 'learnings', 'runs', 'idTag']);
...
function loadPack(name, root) {
  ...
  const candidate = fs.existsSync(builtin) ? builtin : path.resolve(root, name);
...
function load() {
  ...
  const roots = knowledge.map((k, i) => {
    ...
    return { path: path.resolve(root, k.path), rel: k.path, namespace, untagged };
  });
  const seen = new Set();
  ...
  const learnings = path.resolve(root, pick('learnings', 'akela/LEARNINGS.md'));
  const runs = path.resolve(root, pick('runs', '.akela/runs'));
  ...
  cache = {
    ...
    statuses, fingerprints, activities, profile, scratchpad, idTag,
